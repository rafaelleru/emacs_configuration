...
#+RESULTS:
| hrs/reset-font-size | global-hl-todo-mode | global-hl-line-mode | set-font | load-my-theme | x-wm-set-size-hint | table--make-cell-map |
...
** Helm (hay que revisar)
   #+BEGIN_SRC emacs-lisp
     ...
     (helm-mode t)
     (global-set-key (kbd "C-x C-f") 'helm-find-files) //Me gustaria que <TAB> complete y un <del> borre hasta /
